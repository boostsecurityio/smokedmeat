...
|----------|------|--------|-------|------|
| 1 | Operator command discoverability | ✅ | Shipped to `main`. Completion, hints, help, and no-session guidance now match the real `order` surface, including `order exec`. | Task: [docs/tasks/operator-command-discoverability.md](tasks/operator-command-discoverability.md) |
...
| 🔲 | Browser graph filtering and usability | Add scalable filtering and safer default rendering for large graphs, including auto-filtered mode above a size threshold and the ability to hide nodes with no vuln-bearing path. | Task: [docs/tasks/browser-graph-filtering-and-usability.md](tasks/browser-graph-filtering-and-usability.md) |
